...
def test_adapter_success_has_execution_witness():
    ...
    assert ew["backend_id"] == "materials_science"
...
def test_adapter_error_status_failed():
    output = {
        ...
        "error": "missing two_theta",
        "diagnostics": {},
    }
    result = adapt_to_run_result(output, SAMPLE_RUN_CONTEXT)
    assert result["status"] == "failed"
    assert result["error"]["error_code"] == "INVALID_PARAMS"
    assert "two_theta" in result["error"]["message"]
...
def test_adapter_echoes_run_context():
    ...
    result = adapt_to_run_result(output, SAMPLE_RUN_CONTEXT)
    assert result["run_id"] == "run-mat-001"
    assert result["workspace_id"] == "ws-mat-001"
...
def test_ssv_has_all_lowercase_fields():
    ...
    method_result = {"result": {"peaks": [1, 2, 3]}}
    ssv = to_ssv(method_result, "xrd_analysis")
